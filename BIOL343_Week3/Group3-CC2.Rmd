...
```{r import-clean}
cc1_data=read.csv("A1_Data_Excelcopy.csv")
cc1_cleandata=cc1_data[0:45, 0:8]
cc1_cleandata$ovary_area=cc1_cleandata$ovarywidth*cc1_cleandata$ovaryheight

head(cc1_cleandata)

# Should we only included the 5 columns from Challenge 1 or the oeingal with 8 columns
# Challenge 1 we subsetted to have pop type, ovary width, ovary height, flower number, and ovary area
...
# Look for aggregate in Chapter 3
...
```{r area-table, echo=F}
knitr:kable(area_means, col.names=c("Population Type", "Mean Ovary Area", caption="Mean Ovary Area across Asexual (A) and Sexual (S) Population Types"))
...
```{r area-histogram}
ggplot(aes(x = ovary_area, fill = populationtype), data=cc1_cleandata) + geom_histogram(alpha = 0.7) + theme_classic() +  
labs(title = "Ovary Area Between Populations", subtitle = "(Asexual vs. Sexual)", x = "Population Type", y = "Ovary Area (mm²)", fill = "Population")

# facet_wrap() read text on this? 
```
